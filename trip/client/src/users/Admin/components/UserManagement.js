...
const UserManagement = () => {
    ...
    return (
        <div className='mt-8'>
            <h3 className='text-2xl font-black mb-10'>유저 관리</h3>
            <table className='w-10/12 bg-white shadow-sm rounded'>
                    ...
                    {sortedUser.map((user) => (
                        <tr key={user._id} className='hover:bg-gray-50 border-b-3 border-b-2'>
                            ...
                            <td className={`p-1 mt-2.5 inline-block  rounded-lg  ${user.status === 'Active' ? 'bg-green-300' : 'bg-red-300'}`}>
                                <span className='text-sm font-light p-1'>{user.status}</span>
                            </td>
                            <td>
